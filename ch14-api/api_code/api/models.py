...
import os
import secrets
from sqlalchemy import Column, DateTime, Enum, Float, ForeignKey, Integer, Unicode
...
class Train(Base):
    ...
    station_from = relationship(
        "Station", foreign_keys=[station_from_id], back_populates="departures"
    )
...
def _hash(password: str, salt: bytes = None):
    ...
    hashed = hashlib.pbkdf2_hmac(
        "sha256", password.encode("utf-8"), salt, iterations, dklen=128
    )
